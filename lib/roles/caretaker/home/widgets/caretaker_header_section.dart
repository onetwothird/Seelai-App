...
class _HeaderSectionState extends State<HeaderSection>
  ...
  List<String> _getMascotMessages() {
    ...
    if (widget.pendingRequestsCount > 0) {
      messages.add(
        'Hello, ${_getFirstName()}! You have ${widget.pendingRequestsCount} pending request(s). Tap the bell icon to check them out.',
      );
      messages.add(
        'Action Required: You have pending alerts waiting for your response.',
      );
    }

    if (widget.activeRequestsCount > 0) {
      messages.add(
        'Reminder: You currently have ${widget.activeRequestsCount} active request(s) in progress.',
      );
    }

    if (messages.isEmpty) {
      messages.add(
        'Hello, ${_getFirstName()}! You\'re all caught up. I\'m here to help you manage your paired users.',
      );
      messages.add(
        'Did you know? You can toggle dark mode using the moon icon in the top right!',
      );
    }
