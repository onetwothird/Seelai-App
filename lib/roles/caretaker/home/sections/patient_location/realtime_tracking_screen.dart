...
class _RealtimeTrackingScreenState extends State<RealtimeTrackingScreen>
  ...
  bool _hasCaretakerLocationChanged(Map<String, dynamic> newLocation) {
    ...
    double distance = locationTrackingService.calculateDistance(
      ...
      lat2: newLocation['latitude'] as double,
      lon2: newLocation['longitude'] as double,
    );
    return distance > 1.5;
  }
  ...
  void onPatientSelected(PatientModel patient) async {
    ...
    final patientData = await databaseService.getUserDataByRole(
      patient.id,
      'partially_sighted',
    );
    ...
    final location = await locationTrackingService.getPatientLocation(
      patient.id,
    );
    if (location != null && mounted) {
      ...
      if (mounted) {
        _speak('Location is currently not available for ${patient.name}.');
      }
    }
    ...
    try {
      ...
      if (_currentPatientLocation != null &&
        ...
        final patientLatLng = LatLng(
          _currentPatientLocation!['latitude'] as double,
          _currentPatientLocation!['longitude'] as double,
        );
        final patientImageUrl =
            _selectedPatientFullData?['profileImageUrl'] as String?;
        ...
        if (markerBytes != null) {
          newMarkers.add(
            Marker(
              ...
              icon: BitmapDescriptor.bytes(markerBytes),
              zIndexInt: 2,
            ),
          );
        }
      ...
      if (_currentCaretakerLocation != null) {
        final caretakerLatLng = LatLng(
          _currentCaretakerLocation!['latitude'] as double,
          _currentCaretakerLocation!['longitude'] as double,
        );
        final caretakerImageUrl =
            _caretakerFullData?['profileImageUrl'] as String? ??
        ...
        if (markerBytes != null) {
          newMarkers.add(
            Marker(
              markerId: const MarkerId('caretaker'),
              position: caretakerLatLng,
              icon: BitmapDescriptor.bytes(markerBytes),
              zIndexInt: 1,
            ),
          );
  ...
  bool _hasPatientLocationChanged(Map<String, dynamic> newLocation) {
    ...
    double distance = locationTrackingService.calculateDistance(
      lat1: _currentPatientLocation!['latitude'] as double,
      lon1: _currentPatientLocation!['longitude'] as double,
      lat2: newLocation['latitude'] as double,
      lon2: newLocation['longitude'] as double,
    );
    return distance > 1.5;
  ...
  Future<void> _drawRoute({bool frameRoute = true}) async {
    ...
    try {
      ...
      if (frameRoute) {
        double minLat = start.latitude < end.latitude
            ? start.latitude
            : end.latitude;
        double maxLat = start.latitude > end.latitude
            ? start.latitude
            : end.latitude;
        double minLng = start.longitude < end.longitude
            ? start.longitude
            : end.longitude;
        double maxLng = start.longitude > end.longitude
            ? start.longitude
            : end.longitude;
  ...
  Widget _buildMap() {
    return GoogleMap(
      initialCameraPosition: const CameraPosition(
        target: LatLng(14.2456, 121.1234),
        zoom: 17.0,
      ),
      ...
      child: BackdropFilter(
        ...
        child: Container(
          ...
          child: Row(
            children: [
              Icon(
                _isTracking ? Icons.radar : Icons.map_outlined,
                color: _isTracking ? Colors.green : primary,
              ),
              const SizedBox(width: 12),
              Expanded(
                child: Text(
                  _isTracking
                      ? 'Tracking ${_selectedPatient!.name}'
