...
class MswdLocationTrackingScreen extends StatefulWidget {
  ...
  @override
  State<MswdLocationTrackingScreen> createState() =>
      _MswdLocationTrackingScreenState();
}
...
class _MswdLocationTrackingScreenState extends State<MswdLocationTrackingScreen>
  ...
  Future<void> _startLocationTracking() async {
    _locationsStream = mswdLocationTrackingService
        ...
        .listen((locationsData) async {
          ...
          await _processUserLocations(
            locationsData['patients'] ?? [],
            'partially_sighted',
            newLocations,
          );
          await _processUserLocations(
            locationsData['caretakers'] ?? [],
            'caretaker',
            newLocations,
          );
          await _processUserLocations(
  ...
  Future<void> _fetchUserProfile(String userId, String userType) async {
    try {
      final userData = await databaseService.getUserDataByRole(
        userId,
        userType,
      );
      if (userData != null && mounted) {
        setState(() {
          _userProfiles[userId] = {...userData, 'userType': userType};
        });
        if (_isMapReady) _updateMapMarkers();
      }
    } catch (e) {
  ...
  Future<void> _refreshLocationData() async {
    if (!mounted) return;
    try {
      final allLocations = await mswdLocationTrackingService
          .getAllUserLocations();
      Map<String, Map<String, dynamic>> newLocations = {};
    ...
    try {
      ...
      final filteredList = _userLocations.entries.where((entry) {
        if (entry.key == currentUserId) {
          return false; // ✅ SKIP drawing yourself twice!
        }

        final userType = entry.value['userType'] ?? '';
        final isActive = mswdLocationTrackingService.isLocationRecent(
          entry.value,
        );
        if (_showOnlyActive && !isActive) return false;
        if (_selectedFilter == 'patients' && userType != 'partially_sighted') {
          return false;
        }
      ...
      for (var entry in filteredList) {
        ...
        Color ringColor = userType == 'partially_sighted'
            ? Colors.redAccent
            : Colors.blueAccent;
        if (userType == 'mswd' || userType == 'admin') {
          ringColor = primary; // Other Admins
        }
  ...
  Future<void> _drawRouteToUser(LatLng targetPoint) async {
    ...
    try {
      ...
      double minLat = myLocation.latitude < targetPoint.latitude
          ? myLocation.latitude
          : targetPoint.latitude;
      double maxLat = myLocation.latitude > targetPoint.latitude
          ? myLocation.latitude
          : targetPoint.latitude;
      double minLng = myLocation.longitude < targetPoint.longitude
          ? myLocation.longitude
          : targetPoint.longitude;
      double maxLng = myLocation.longitude > targetPoint.longitude
          ? myLocation.longitude
          : targetPoint.longitude;
      ...
      if (mounted) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text(
              'Failed to calculate route. Please ensure your location is enabled.',
            ),
          ),
        );
      }
  ...
  Future<void> _selectUser(String userId) async {
    ...
    if (location != null) {
      ...
      await _mapController?.animateCamera(
        CameraUpdate.newLatLngZoom(LatLng(targetLat, targetLng), 17.0),
      );
  ...
  Widget _buildUserCarousel(dynamic theme) {
    ...
    final usersList = _userLocations.values.where((loc) {
      if (loc['userId'] == currentUserId) {
        return false; // ✅ Hide yourself from the carousel
      }

      final userType = loc['userType'];
      if (_showOnlyActive && !mswdLocationTrackingService.isLocationRecent(loc)) {
        return false;
      }
      if (_selectedFilter == 'patients' && userType != 'partially_sighted') {
        return false;
                    ...
                    if (phone.isNotEmpty) {
                      ...
                      if (mounted) {
                        ScaffoldMessenger.of(context).showSnackBar(
                          const SnackBar(
                            content: Text(
                              'No contact number available for this user.',
                            ),
                          ),
                        );
                      }
