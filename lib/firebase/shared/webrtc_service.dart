...
class WebRTCService {
  ...
  Future<void> openUserMedia(bool isVideo) async {
    ...
    try {
      ...
      debugPrint(
        "Ideal camera request failed: $e. Retrying with basic constraints...",
      );
      _localStream = await navigator.mediaDevices.getUserMedia({
        'audio': true,
  ...
  Future<void> makeCall(String path, String callId, bool isVideo) async {
    ...
    _database.ref('$path/calls/$callId/receiverCandidates').onChildAdded.listen(
      (event) {
        if (event.snapshot.exists) {
          final data = Map<String, dynamic>.from(event.snapshot.value as Map);
  ...
  Future<void> answerCall(String path, String callId, bool isVideo) async {
    ...
    _database.ref('$path/calls/$callId/callerCandidates').onChildAdded.listen((
      event,
    ) {
      if (event.snapshot.exists) {
        final data = Map<String, dynamic>.from(event.snapshot.value as Map);
