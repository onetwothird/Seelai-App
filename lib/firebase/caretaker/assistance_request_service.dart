...
class AssistanceRequestService {
  ...
  Future<RequestModel?> getRequestById(String requestId) async {
    try {
      final event = await _database.ref('assistance_requests/$requestId').once();
  ...
  Stream<List<RequestModel>> streamCaretakerRequests(String caretakerId) {
    return _database.ref('assistance_requests').orderByChild('caretakerId').equalTo(caretakerId).onValue.map((event) {
  ...
  Stream<List<RequestModel>> streamPatientRequests(String patientId) {
    return _database.ref('assistance_requests').orderByChild('patientId').equalTo(patientId).onValue.map((event) {
      if (!event.snapshot.exists) return <RequestModel>[];
  ...
  Future<bool> declineRequest(String requestId, String caretakerId, String reason) async {
    try {
      await _database.ref('assistance_requests/$requestId').update({
        'status': 'pending', 
        'caretakerId': null, 
  ...
  Future<bool> completeRequest(String requestId, String caretakerId, String notes) async {
    try {
      await _database.ref('assistance_requests/$requestId').update({
        'status': 'completed',
  ...
  Future<List<RequestModel>> getRequestsByStatus(String caretakerId, RequestStatus status) async {
    try {
      final allRequests = await getCaretakerRequests(caretakerId);
